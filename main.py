...
class JobTracker:
    ...
    def _load_global_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Global configuration file {self.config_path} does not exist"
            )
    ...
    def _save_site_snapshot(self, website_name: str, jobs: List[Dict[str, Any]]) -> None:
        ...
        Otherwise, falls back to data/<site>jobs.json
        """
        if not jobs:
            return

        try:
            # Optional mapping in config: {"wbd": "data/wbdjobs.json", ...}
            site_map = (
                self.config.get("global_settings", {}).get("site_output_map", {}) or {}
            )
            ...
            self.logger.info(
                f"Wrote {len(jobs)} {website_name} jobs to snapshot: {out_path}"
            )
        except Exception as e:
            self.logger.error(f"Failed to write {website_name} snapshot: {e}")
    ...
    async def _run_website_scraper(self, website_name: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            scraper_class_name = f"{website_name.title().replace('_', '')}Scraper"
    ...
    async def run_all_scrapers(self) -> None:
        ...
        for website in enabled_websites:
            try:
                ...
                if global_delay > 0:
                    self.logger.info(
                        f"Waiting {global_delay} seconds before continuing..."
                    )
                    await asyncio.sleep(global_delay)

            except Exception as e:
                self.logger.error(
                    f"Error occurred while processing website {website}: {e}"
                )
                continue

        # ✅ If only one site is enabled, skip writing to global jobs.json
        if len(enabled_websites) > 1:
            ...
            self.logger.info(
                f"Skipping global output since only {enabled_websites[0]} is enabled"
            )
